...
public class PlayerMovement : MovementTracker
{
    ...
    private void handleWalkingStateKeyPresses()
    {
        ...
        if (Input.GetKey(KeyBindingList.removePlacedCompanionMovableObjectKey) && !KeyPressManager.handlingPrimaryKeyPress)
        {
            ...
            else if (Physics2D.OverlapCircle(colliderWorldPosition(), detectionSize, LayerAndTagManager.moveableObjectLayerMask))
            {
                GameObject movableObject = Physics2D.OverlapCircle(colliderWorldPosition(), detectionSize, LayerAndTagManager.moveableObjectLayerMask).gameObject;
    ...
    public bool handleWASDMovement()
    {
        ...
        if (currentMovementKeyCode != KeyCode.None && Input.GetKey(currentMovementKeyCode) && !isMoving())
        {
            ...
            if (Physics2D.OverlapCircle(colliderWorldPosition(), detectionSize, LayerAndTagManager.moveableObjectLayerMask))
            {

                if (!Helpers.checkPositionForColliders(colliderWorldPosition(2), detectionSize, LayerAndTagManager.blocksMoveableObjectLayerMask))
                {
    ...
    private string getPromptMessage()
    {
        ...
        collider = Physics2D.OverlapCircle(instance.colliderWorldPosition(), detectionSize, LayerAndTagManager.moveableObjectLayerMask);
