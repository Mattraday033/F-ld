...
public static class ItemList 
{
	...
	private static void initializeItemList()
	{
		...
		usableItems.Add(new RestorationItem(new ItemListID(usableItemListIndex, bandagesIndex), "Bandages", "Wrappings made from a coarse, firm cloth.", "Used to remove all " + TraitType.Wound.ToString() + " Traits from a friendly target.", ItemSpriteList.bandagesSprite, 15, SelectorTemplate.BoxThree, TraitType.Wound));
		...
		//Weapon(string key, string loreDescription, string damageFormula, string critFormula, string iconName, SelectorTemplate rangeTemplate, int worth, int slotID)
		//Weapon(string key, string loreDescription, string damageFormula, string critFormula, string iconName, SelectorTemplate rangeTemplate, int worth, int slotID, bool isTwoHanded)

		weapons.Add(new Weapon(new WeaponListID(cudgelIndex), "Cudgel", "A wooden club made from a fallen tree branch.", "2S + 5", "S+D", ItemSpriteList.cudgelSprite, SelectorTemplate.VerticalOne, 3, isOneHanded, EffectAnimationType.Blunt));
		weapons.Add(new Weapon(new WeaponListID(shivIndex), "Shiv", "A weapon made from a bronze nail tied to a small piece of wood.", "2D + 7", "3D", ItemSpriteList.shivSprite, SelectorTemplate.Single, 3, isOneHanded, EffectAnimationType.Pierce));
		weapons.Add(new Fist(new WeaponListID(mainHandFistIndex), dominantFistKey, "Good old fashioned meat bludgeons.", "S+D+W", "D+W", "FistIcon", SelectorTemplate.Single));
        weapons.Add(null);
		weapons.Add(new Weapon(new WeaponListID(malletIndex), "Mallet", "A large hammer used to beat pitons into rock walls.", "3S + 6", "D", ItemSpriteList.malletSprite, SelectorTemplate.HorizontalOne, 15, isOneHanded, EffectAnimationType.Blunt));
		weapons.Add(new Weapon(new WeaponListID(lightPickIndex), "Light Pick", "A bronze pick meant to be used in one hand.", "3D + 8", "3D", ItemSpriteList.oneHandedPickSprite, SelectorTemplate.HorizontalOne, 15, isOneHanded, EffectAnimationType.Pierce));
		weapons.Add(new Weapon(new WeaponListID(heavyPickIndex), "Heavy Pick", "A large bronze pick meant to be used in two hands.", "4S + 7", "D", ItemSpriteList.twoHandedPickSprite, SelectorTemplate.HookOne, 15, isTwoHanded, EffectAnimationType.Pierce));
		weapons.Add(new Weapon(new WeaponListID(bronzeBarIndex), "Bronze Bar", "A long thin bronze ingot. A bit oxidized, but hefty.", "3S + 8", "D + 2", ItemSpriteList.bronzeBarSprite, SelectorTemplate.HorizontalThree, 5, isTwoHanded, EffectAnimationType.Blunt));
		weapons.Add(null);
		weapons.Add(new Fist(new WeaponListID(improvedMainHandFistIndex), dominantFistKey, "Good old fashioned meat bludgeons.", "2W+S+D+4", "W+D+2", "ImprovedFistIcon", SelectorTemplate.VerticalOne));
		weapons.Add(new Fist(new WeaponListID(greaterMainHandFistIndex), dominantFistKey, "Good old fashioned meat bludgeons.", "3W+S+D+8", "W+D+4", "GreaterFistIcon", SelectorTemplate.BoxOne));
		weapons.Add(new Fist(new WeaponListID(ruinousMainHandFistIndex), dominantFistKey, "Good old fashioned meat bludgeons.", "4W+S+D+16", "W+D+8", "RuinousFistIcon", SelectorTemplate.Single)); //When implementing 6 range selectors, this gets Sextuple Box/Horizontal
		...
		weapons.Add(new Weapon(new WeaponListID(sharpRockIndex), "Sharp Rock", "A stone, chipped to have a meager edge. ", "2C + 3", "C+D", ItemSpriteList.sharpRockSprite, SelectorTemplate.Single, 1, isOneHanded, EffectAnimationType.Pierce));
        weapons.Add(new StanceWeapon(new WeaponListID(thinBladeIndex), "Bronze <nobr>Thin-Blade</nobr>", "A long, slender, double-edged blade with no crossguard. Favored by swordsmen for its long reach, they are either wielded solo or with a matching dagger.", "2W+2D+6", "D+W+1", ItemSpriteList.thinbladeSprite, SelectorTemplate.VerticalOne, worth: 55, isOneHanded, EffectAnimationType.Slash));
		weapons.Add(new Weapon(new WeaponListID(scaldIndex), "Scald", "Kende's trusty frying pan. Those struck with it come away burned.", "2S+2D+2C+9", "S+D+C", ItemSpriteList.fryingPanSprite, SelectorTemplate.HorizontalOne, worth: 75, isOneHanded, EffectAnimationType.Blunt, traitToApply: TraitList.roasted));
		weapons.Add(new Weapon(new WeaponListID(whipIndex), "Whip", "A coiled snake of pain.", "2S+2D+10", "S+D", ItemSpriteList.whipSprite, SelectorTemplate.VerticalThree, worth: 35, isOneHanded, EffectAnimationType.Slash, traitToApply: TraitList.wounded));
