...
public static class ItemList 
{
	...
	private static void initializeItemList()
	{
		...
		usableItems.Add(new RestorationItem(new ItemListID(usableItemListIndex, bandagesIndex), "Bandages", "Wrappings made from a coarse, firm cloth.", "Used to remove all " + TraitType.Wound.ToString() + " Traits from a friendly target.", ItemSpriteList.bandagesSprite, 15, SelectorList.boxThreeName, TraitType.Wound));
		...
		weapons.Add(new Fist(new WeaponListID(mainHandFistIndex), dominantFistKey, "Good old fashioned meat bludgeons.", "S+D+W", "D+W", "FistIcon", SelectorList.singleName));
        weapons.Add(null);
		weapons.Add(new Weapon(new WeaponListID(malletIndex), "Mallet", "A large hammer used to beat pitons into rock walls.", "3S + 6", "D", ItemSpriteList.malletSprite, SelectorList.horizontalOneName, 15, isOneHanded, EffectAnimationType.Blunt));
		weapons.Add(new Weapon(new WeaponListID(lightPickIndex), "Light Pick", "A bronze pick meant to be used in one hand.", "3D + 8", "3D", ItemSpriteList.oneHandedPickSprite, SelectorList.horizontalOneName, 15, isOneHanded, EffectAnimationType.Pierce));
		weapons.Add(new Weapon(new WeaponListID(heavyPickIndex), "Heavy Pick", "A large bronze pick meant to be used in two hands.", "4S + 7", "D", ItemSpriteList.twoHandedPickSprite, SelectorList.hookOneName, 15, isTwoHanded, EffectAnimationType.Pierce));
		weapons.Add(new Weapon(new WeaponListID(bronzeBarIndex), "Bronze Bar", "A long thin bronze ingot. A bit oxidized, but hefty.", "3S + 8", "D + 2", ItemSpriteList.bronzeBarSprite, SelectorList.horizontalThreeName, 5, isTwoHanded, EffectAnimationType.Blunt));
		weapons.Add(null);
		weapons.Add(new Fist(new WeaponListID(improvedMainHandFistIndex), dominantFistKey, "Good old fashioned meat bludgeons.", "2W+S+D+4", "W+D+2", "ImprovedFistIcon", SelectorList.verticalOneName));
		weapons.Add(new Fist(new WeaponListID(greaterMainHandFistIndex), dominantFistKey, "Good old fashioned meat bludgeons.", "3W+S+D+8", "W+D+4", "GreaterFistIcon", SelectorList.boxOneName));
		weapons.Add(new Fist(new WeaponListID(ruinousMainHandFistIndex), dominantFistKey, "Good old fashioned meat bludgeons.", "4W+S+D+16", "W+D+8", "RuinousFistIcon", SelectorList.singleName)); //When implementing 6 range selectors, this gets Sextuple Box/Horizontal
		weapons.Add(new Weapon(new WeaponListID(bronzeGreatspearIndex), "Bronze Greatspear", "A long spear with a bronze tip, made to be wielded in two hands.", "6S+12", "S+D", ItemSpriteList.bronzeSpearSprite, SelectorList.verticalThreeName, 50, isTwoHanded, EffectAnimationType.Pierce));
		weapons.Add(new Weapon(new WeaponListID(wornBowIndex), "Worn Bow", "This bow is a little weathered, but can still answer the call of it's wielder", "7D+16", "3D", ItemSpriteList.wornBowSprite, SelectorList.verticalOneName, 55, isTwoHanded, EffectAnimationType.Pierce));
		weapons.Add(new Weapon(new WeaponListID(fightingCapeIndex), "Fighting Cape", "A cape wrapped around the mainhand, used to both deflect small blows and disorient the opponent. Often paired with a dagger in the offhand.", "3C+4", "C", ItemSpriteList.capeSprite, SelectorList.singleName, 35, isOneHanded, EffectAnimationType.Slash));
		weapons.Add(new Staff (new WeaponListID(staffIndex), "Staff", "A weathered length of oak-spar. It would serve as well as a walking aid or a bludgeon. Has high Base Damage.", "W+12", "D+W", ItemSpriteList.staffSprite, SelectorList.horizontalOneName, 10, isTwoHanded));
		weapons.Add(null);
		weapons.Add(null);
		weapons.Add(new Staff (new WeaponListID(plankIndex), "Plank", "A long piece of wood, pulled from a shack wall. Poorly balanced, but it'll do in a pinch.", "2W + 2", "W+D", ItemSpriteList.plankSprite, SelectorList.singleName, 3, isOneHanded));
		weapons.Add(new Weapon(new WeaponListID(sharpRockIndex), "Sharp Rock", "A stone, chipped to have a meager edge. ", "2C + 3", "C+D", ItemSpriteList.sharpRockSprite, SelectorList.singleName, 1, isOneHanded, EffectAnimationType.Pierce));
        weapons.Add(new StanceWeapon(new WeaponListID(thinBladeIndex), "Bronze <nobr>Thin-Blade</nobr>", "A long, slender, double-edged blade with no crossguard. Favored by swordsmen for its long reach, they are either wielded solo or with a matching dagger.", "2W+2D+6", "D+W+1", ItemSpriteList.thinbladeSprite, SelectorList.verticalOneName, worth: 55, isOneHanded, EffectAnimationType.Slash));
		weapons.Add(new Weapon(new WeaponListID(scaldIndex), "Scald", "Kende's trusty frying pan. Those struck with it come away burned.", "2S+2D+2C+9", "S+D+C", ItemSpriteList.fryingPanSprite, SelectorList.horizontalOneName, worth: 75, isOneHanded, EffectAnimationType.Blunt, traitToApply: TraitList.roasted));
		weapons.Add(new Weapon(new WeaponListID(whipIndex), "Whip", "A coiled snake of pain.", "2S+2D+10", "S+D", ItemSpriteList.whipSprite, SelectorList.verticalThreeName, worth: 35, isOneHanded, EffectAnimationType.Slash, traitToApply: TraitList.wounded));
