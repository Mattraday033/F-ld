...
public static class AbilityList
{
	...
	private static void instantiateEnemyAbilities()
	{
        ...
        enemyAbilityDictionary.Add(squadStrikeKey, new SquadAbility(CombatActionSettings.build(DescriptionParams.build(squadStrikeKey, "An attack that utilizes the cooperation of other squad members. Deals more damage if the attacker is adjacent to one or more allies."), DamageParams.build("5", "5"), TargetParams.build(Range.boxOneIndex)), "18"));
        enemyAbilityDictionary.Add(skewerKey, new Ability(CombatActionSettings.build(DescriptionParams.build(skewerKey, "The lancer pierces multiple targets in a row."), DamageParams.build("16", "10"), TargetParams.build(Range.verticalThreeIndex), AnimationParams.build(EffectAnimationType.Pierce))));
        enemyAbilityDictionary.Add(executeKey, new Ability(CombatActionSettings.build(DescriptionParams.build(executeKey, "A chop so quick, it may very well be the last thing you ever see."), DamageParams.build("11", "50"), TargetParams.build(Range.horizontalThreeIndex))));
